...
/**
 * Tanimoto Measure (histogram intersection family) between two histograms.
 * 
 * @author Devin Bonnie
 * 
 */
public class TanimotoDistanceMeasure implements Measure, HasCategory {
	...
	/**
	 * Compares two RGB Histograms by Tanimoto Measure. 
	 * 
	...
	public SimilarityNumber compare(RGBHistogramDescriptor feature1, RGBHistogramDescriptor feature2) throws Exception {
		...
		for (int i=0; i < normHist1.length; i++){
			for (int j=0; j < normHist1[0].length; j++){
				...
				s1[j] += ( Math.max( normHist1[i][j], normHist2[i][j] ) - Math.min( normHist1[i][j], normHist2[i][j] ) );
				s2[j] += Math.max( normHist1[i][j], normHist2[i][j] );
			}
		}		
	...
	/**
	 * Compares two Grayscale Histograms by Tanimoto Measure.
	 * 
	...
	public SimilarityNumber compare(GrayscaleHistogramDescriptor feature1, GrayscaleHistogramDescriptor feature2) throws Exception {
		...
		for (int i=0; i < normHist1.length; i++){
			...
			s1 += ( Math.max( normHist1[i], normHist2[i] ) - Math.min( normHist1[i], normHist2[i] ) );
			s2 += Math.max( normHist1[i], normHist2[i] );
		}
	...
	public String getName() {
		return "Tanimoto Measure (Intersection Family)";
	}
	...
	@Override
	public Class<TanimotoDistanceMeasure> getType() {
		return TanimotoDistanceMeasure.class;
	}
